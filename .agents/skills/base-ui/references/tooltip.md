...
- `Tooltip` adds `closeOnClick` for flows where clicking the trigger or target action should also dismiss the hint.
- Keep using tooltips as non-essential hints for sighted users; do not move required form or validation messaging into tooltip-only content.
...
export default function TooltipDetachedTriggersControlledDemo() {
  ...
  return (
    <Tooltip.Provider>
      ...
      <Tooltip.Root handle={demoTooltip} open={open} onOpenChange={handleOpenChange} triggerId={triggerId}>
        <Tooltip.Portal>
          <Tooltip.Positioner
            className="
              h-(--positioner-height)
          ...
          <Tooltip.Portal>
            <Tooltip.Positioner sideOffset={10} className={styles.Positioner}>
              <Tooltip.Popup className={styles.Popup}>
                ...
                <Tooltip.Viewport className={styles.Viewport}>{Payload !== undefined && <Payload />}</Tooltip.Viewport>
              </Tooltip.Popup>
            </Tooltip.Positioner>
          </Tooltip.Portal>
        )}
