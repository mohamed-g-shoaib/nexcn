...
export function getNextOverlayFiles(
  ...
  const files = new Map<string, string>([
    ...
    [path.join(projectDirectory, "components", "starter-shell.tsx"), getStarterShellTemplate(context.config.rtl)],
    [path.join(projectDirectory, "hooks", "use-locale.tsx"), getLocaleHookTemplate(context.config.rtl)],
    [path.join(projectDirectory, "hooks", "use-ui-sound.ts"), getUiSoundHookTemplate()],
    [path.join(projectDirectory, "lib", "i18n.ts"), getI18nTemplate(context.config.rtl)],
  ...
  if (context.config.rtl) {
    files.set(path.join(projectDirectory, "app", "[locale]", "layout.tsx"), getLayoutTemplate(true));
    files.set(path.join(projectDirectory, "app", "[locale]", "page.tsx"), getPageTemplate());
    files.set(path.join(projectDirectory, "components", "language-toggle.tsx"), getLanguageToggleTemplate());
    files.set(path.join(projectDirectory, "proxy.ts"), getProxyTemplate());
    return files;
  }

  files.set(path.join(projectDirectory, "app", "layout.tsx"), getLayoutTemplate(false));
