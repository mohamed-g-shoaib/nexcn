...
export function getStartOverlayFiles(
  ...
  const files = new Map<string, string>([
    [path.join(projectDirectory, "src", "routes", "__root.tsx"), getRootRouteTemplate(context.config.projectName, context.config.rtl)],
    [path.join(projectDirectory, "src", "routes", "index.tsx"), getIndexRouteTemplate(context.config.rtl)],
    ...
    [path.join(projectDirectory, "src", "components", "starter-shell.tsx"), getStarterShellTemplate(context.config.rtl)],
    [path.join(projectDirectory, "src", "hooks", "use-locale.tsx"), getLocaleHookTemplate(context.config.rtl)],
    [path.join(projectDirectory, "src", "hooks", "use-ui-sound.ts"), getUiSoundHookTemplate()],
    [path.join(projectDirectory, "src", "lib", "i18n.ts"), getI18nTemplate(context.config.rtl)],
  ...
  if (context.config.rtl) {
    files.set(path.join(projectDirectory, "src", "routes", "$locale", "route.tsx"), getLocaleLayoutRouteTemplate());
    files.set(path.join(projectDirectory, "src", "routes", "$locale", "index.tsx"), getLocaleIndexRouteTemplate());
    files.set(path.join(projectDirectory, "src", "components", "language-toggle.tsx"), getLanguageToggleTemplate());
    return files;
  }
